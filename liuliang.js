...
!(async () => {
        ...
        if ($.isNode() &&date.getHours() == 11 && date.getMinutes()<10) {
            if (message.length != 0) {
                   await notify.sendNotify("流量来了签到", `${message}\n\n `);
            }
        } else {
            $.msg($.name, "",  message)
        }
...
//获取签到body
function getllllbody() {
    if ($request.url.indexOf("userSign") > -1) {
          bodyVal = $request.body
        if (llllbody) {
            if (llllbody.indexOf(bodyVal) > -1) {
                $.log("此签到请求已存在，本次跳过")
            } else if (llllbody.indexOf(bodyVal) == -1) {
                llllbodys = llllbody + "&" + bodyVal;
                $.setdata(llllbodys, 'llllbody');
                $.log(`${$.name}获取签到: 成功, llllbodys: ${bodyVal}`);
                bodys = llllbodys.split("&")
                $.msg($.name, "获取第" + bodys.length + "个签到请求: 成功??", ``)
            }
        } else {
            $.setdata(bodyVal, 'llllbody');
            $.log(`${$.name}获取签到: 成功, llllbodys: ${bodyVal}`);
